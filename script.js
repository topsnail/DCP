...
class CSVDataApp {
    ...
    bindEventListeners() {
        ...
        if (exportBtn) {
            exportBtn.addEventListener('click', () => this.exportToXlsx());
        }
    ...
    handleFileImport(event) {
        ...
        const validMimeTypes = ['text/csv', 'application/csv', 'text/comma-separated-values', 'application/vnd.ms-excel'];
        ...
        const isValidType = validMimeTypes.includes(fileType) || fileType === '';
        
        if (!isValidExtension || !isValidType) {
            this.showToast('请选择有效的CSV文件！\n\n提示：\n1. 确保文件扩展名为.csv\n2. 检查文件是否被其他程序占用\n3. 尝试使用Chrome浏览器', 'error');
            // 清空文件输入
            event.target.value = '';
            return;
        }
    ...
    exportToXlsx() {
        ...
        try {
            ...
            // 表头
            const headers = ['日期', '时间', '孔号', '杆号', '锤击数', '深度', '总深度'];
            
            // 转换数据格式
            const rawDataSheetData = [headers];
            ...
            exportData.forEach(item => {
                ...
                rawDataSheetData.push(row);
            });
            
            // 创建工作表
            const worksheet = XLSX.utils.aoa_to_sheet(rawDataSheetData);
            ...
            statsSheetData.push(['统计信息']);
            statsSheetData.push(['总数据行数:', totalRows]);
            statsSheetData.push(['孔号数量:', holeCount]);
            statsSheetData.push(['当前显示行数:', displayedRows]);
            statsSheetData.push(['']); // 空行分隔
            
            // 各孔号最大深度及对应总深度（与网页一致）
            statsSheetData.push(['各孔号最大深度及对应总深度']);
            statsSheetData.push(['孔号', '总锤击数', '总深度', '最大贯入深度', '对应锤击数']);
            ...
            holeNumbersToShow.forEach(holeNumberStr => {
                ...
                // 添加孔号统计行（与网页显示格式一致）
                statsSheetData.push([`孔号: ${holeNumber}`, '', '', '', '']);
                statsSheetData.push(['总锤击数', totalBlowCount, '总深度', `${totalDepth} mm`, '']);
                
                // 添加最大深度标题
                statsSheetData.push(['最大贯入深度', `${maxDepth} mm`, '', '', '']);
                
                // 添加每个最大深度记录（与网页显示格式一致）
                maxDepthRecords.forEach((record, index) => {
                    const maxDepthBlowCount = record.blowCount;
                    const maxDepthTotalDepth = record.totalDepth;
                    
                    statsSheetData.push(['第', `${maxDepthBlowCount}`, '次锤击', `贯入${maxDepth} mm/总深${maxDepthTotalDepth} mm`, '']);
                });
            ...
            // 添加图表说明
            statsSheetData.push(['']);
            statsSheetData.push(['图表说明:', 'Excel无法直接嵌入动态生成的JavaScript图表']);
            statsSheetData.push(['', '您可以使用原始数据工作表中的数据，在Excel中使用']);
            statsSheetData.push(['', '插入图表功能手动生成曲线图。']);
            
            // 将统计信息从I1单元格开始添加到工作表
            XLSX.utils.sheet_add_aoa(worksheet, statsSheetData, { origin: 'I1' });
            ...
            this.showToast(`数据导出失败: ${error.message}`, 'error');
        }
    }
}
